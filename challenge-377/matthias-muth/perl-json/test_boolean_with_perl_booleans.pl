...
use if $] >= 5.036, builtin => qw( true false is_bool );
no  if $] >= 5.036, warnings => 'experimental::builtin';
use if $] < 5.036,  boolean => qw( true false isBoolean );
BEGIN { *is_bool = \&boolean::isBoolean if $] < 5.036 }
...
### FOR TESTING:
### Add tests using Perl builtin boolean values (from 'builtin' or 'boolean'):
push @{ $json_data->{examples} },
    { name => "Perl true is true",   in => [ true ],  out => [ true  ] },
    { name => "Perl false is false", in => [ false ], out => [ false ] };
