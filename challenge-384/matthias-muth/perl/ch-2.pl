...
sub special_binary_substrings_substr_map( $binary ) {
    return uniq map {
...
sub special_binary_substrings_destructive( $binary ) {
    my ( @results, %marked );
    for ( 0 .. length( $binary ) - 2 ) {
        if ( $binary =~ / (0(?-1)*1) | (1(?-1)*0) /gx
            && ! $marked{$&} )
...
my @benchmark_data = ( "10011100" );

run( "special_binary_substrings", \@tests, \@benchmark_data );
