...
sub consecutive_sequence_using_hash( @ints ) {
    ...
    for my $n ( uniq @ints ) {
        ...
        # Update the lookups with the new streak.
        $left  and delete $streaks{$left->{TO}};
        $right and delete $streaks{$right->{FROM}};
...
sub consecutive_sequence_using_class( @ints ) {
    ...
    for my $n ( uniq @ints ) {
        ...
        # to the left or to the right.
        for ( $streaks{ $_ - 1 }, $streaks{ $_ + 1 } ) {
            next unless $_;

            $streak->merge( $_ );

            # Update the lookups.
            $left  and delete $streaks{$left->end};
            $right and delete $streaks{$right->start};
            $streaks{ $streak->start } =
                $streaks{ $streak->end } = $streak;

            # Update the maximum length.
            $max_streak_length =
                max( $max_streak_length, $streak->end - $streak->start + 1 );
        }
    }
    return $max_streak_length;
}
...
is consecutive_sequence( 10, 4, 20, 1, 3, 2 ), 4,
    'Example 1: consecutive_sequence( 10, 4, 20, 1, 3, 2 ) == 4';
is consecutive_sequence( 0, 6, 1, 8, 5, 2, 4, 3, 0, 7 ), 9,
    'Example 2: consecutive_sequence( 0, 6, 1, 8, 5, 2, 4, 3, 0, 7 ) == 9';
is consecutive_sequence( 10, 30, 20 ), -1,
    'Example 3: consecutive_sequence( 10, 30, 20 ) == -1';
