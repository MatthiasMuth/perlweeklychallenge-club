...
* For finding the index with the closest distance, we add some 'traditional' minimum finding code. We need to keep the currently best index as well as the current closest distance, and update after a comparison  in the loop.
...
##### The Shorter Solution (using a clever library function)
 ...
 Normally it is much easier to use a `min` function than to program out a for loop for finding a minimum.
...
We can feed it the indexes of the 'valid' points, and let the `KEYFUNC` calculate the Manhattan distance as the criteria for the minimum.<br/>To get the indexes of the 'valid' points we filter them from the original list of points, using `grep`.

As we separate the filtering from the distance calculation, we can't do the shortcuts for the x- or y-coordinates equaling `$x` or `$y` here. We need to do the standard Manhattan distance calculation that includes both dimensions.

Still this results in a much shorter solution that avoids the `for` loop and nicely hides that two-variable minimum calculation:
